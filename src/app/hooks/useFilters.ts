...
export const useFilters = () => {
	...
	const active = params.active ? JSON.parse(params.active) : false;
	const promotion = params.promotion ? JSON.parse(params.promotion) : false;
	...
	const handleSubmit = (values: FormValues) => {
		...
		const filterUrl = createQueryUrl({
			searchParams: { ...params, ...queryParams, page: '1' },
		}) as URL;
